...
if (isset($_POST['submit'])) {
	...
	else
	{
		...
		else
		{
			...
			$sql = "INSERT INTO items (Item_id, Item_name, Category, Price, Quantity, Description, Image)
			VALUES ('$id', '$name', '$category', '$price', '$quantity', '$desc', '$newImageName')";
			mysqli_query($conn, $sql);
			
			echo "<script>alert('Successfully Added');
				  document.location.href = 'testmanage.php';
				  </script>";
    ...
    // $sql = "INSERT INTO items (Item_id, Item_name, Category, Price, Quantity, Description, Image)
            // VALUES ('$id', '$name', '$category', '$price', '$quantity', '$desc', '$escapedImageData')";

    // $result = mysqli_query($conn, $sql);
    // if ($result) {
        // header("location:testmanage.php");
    // } else {
    ...
    <div class="container">
        <form method="post" enctype="multipart/form-data" action="testadd.php">
            ...
            <div class="form">
                ...
                <input type="text" class="form-cntrl" id="category" placeholder="Enter Category" name="category" required>
            </div>
            <div class="form">
